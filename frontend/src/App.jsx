    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-lg border border-slate-200 bg-white p-5 shadow-xl">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={onConfirm} className="rounded-md bg-rose-600 px-3 py-1.5 text-sm font-medium text-teal-600 shadow-sm hover:bg-rose-700">{confirmText}</button>
        </div>
      </div>
    </div>
  );
...
// UI: DatePicker (custom popover calendar)
function DatePicker({ value, onChange, className = "", containerClass = "" }) {
  ...
  return (
    <div className={"relative " + containerClass}>
      ...
      >
        <span className={value ? "text-slate-800" : "text-slate-400"}>{formatDisplayDate(value)}</span>
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-72 overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl" style={{ animation: "fadeScaleIn 120ms ease-out" }}>
              ...
              return (
                <button
                  ...
                  className={
                    ...
                    (selected ? "bg-teal-50 text-teal-700" : inMonth ? "text-slate-800 hover:bg-slate-100" : "text-slate-400") +
          ...
          <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2 text-sm">
            <button className="text-slate-500 hover:text-slate-700" onClick={() => { onChange(""); setOpen(false); }}>Clear</button>
            <div className="flex items-center gap-2">
              <button className="text-teal-600 hover:text-teal-700 font-medium" onClick={() => { onChange(toISODateString(today)); setY(today.getFullYear()); setM(today.getMonth()); setOpen(false); }}>Today</button>
              <button className="text-teal-600 hover:text-teal-700 font-medium" onClick={() => { const now = new Date(); const start = new Date(now); start.setDate(now.getDate() - now.getDay()); onChange(toISODateString(start)); setY(start.getFullYear()); setM(start.getMonth()); setOpen(false); }}>This week</button>
              <button className="text-teal-600 hover:text-teal-700 font-medium" onClick={() => { const start = new Date(today.getFullYear(), today.getMonth(), 1); onChange(toISODateString(start)); setY(start.getFullYear()); setM(start.getMonth()); setOpen(false); }}>This month</button>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  const [query, setQuery] = useState({ search: "", status: "", from: "", to: "", page: 1, limit: 5 });
  ...
  const onStatusChange = async (id, status) => {
    ...
    try {
      ...
      setData(d => ({ ...d, items: d.items.map(t => t._id === id ? { ...t, status } : t) }));
      setStats(s => ({ ...s, [status]: (s[status] || 0) + 1 }));
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      <h1 className="text-3xl font-semibold tracking-tight text-slate-800 text-center mb-6">TaskNet</h1>
